...
public final class AssetCell: UICollectionViewCell {
    ...
    public private(set) lazy var boxCoverView: UIView = {
        let view = UIView(frame: .zero)
        view.isHidden = true
        view.layer.borderWidth = 4
        return view
    }()
    public private(set) lazy var selectButton: NumberCircleButton = {
...
//        }
        ...
        selectButton.snp.makeConstraints { maker in
            maker.top.right.equalToSuperview().inset(-16)
...
extension AssetCell {
    ...
    public func setContent(_ asset: Asset, manager: PickerManager, animated: Bool = false, isPreview: Bool = false) {
        self.asset = asset
        ...
        manager.requestPhoto(for: asset.phAsset, options: options, completion: { [weak self, weak asset] result in
            guard let self = self, let asset = asset else { return }
            switch result {
            case .success(let response):
                guard self.identifier == asset.phAsset.localIdentifier else { return }
                if let image = asset._image {
                    self.imageView.image = image
                } else {
                    asset.__image = response.image
    ...
    public func updateState(_ asset: Asset, manager: PickerManager, animated: Bool = false, isPreview: Bool = false) {
