...
extension LTScrollView: UIGestureRecognizerDelegate {
    public override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if bounces == false {
            ...
            if trans.y < 0 {
                if self.contentOffset.y - self.contentInset.bottom >= round( self.contentSize.height - self.bounds.size.height) {
                    return false
                }
            }
            if trans.x > 0 {
            ...
            if trans.x < 0 {
                if self.contentOffset.x - self.contentInset.right >= round( self.contentSize.width - self.bounds.size.width) {
                    return false
                }
            }
        }
