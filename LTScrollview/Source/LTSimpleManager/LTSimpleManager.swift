 ...
 public class LTSimpleManager: UIView {
    ...
    public var tableviewInset:UIEdgeInsets?
    ...
    internal lazy var tableView: LTTableView = {
    ...
    private func contentScrollViewScrollConfig(_ viewController: UIViewController) {
        viewController.glt_scrollView?.scrollHandle = {[weak self] scrollView in
            ...
            if self.tableView.contentOffset.y  < self.kHeaderHeight - self.hoverY {
                ...
                scrollView.contentOffset = CGPoint(x: 0 - contentInset.left, y: 0 - contentInset.top)
    ...
    private func setupRefreshData()  {
        DispatchQueue.main.after(0.001) {
            UIView.animate(withDuration: 0.34, animations: {
                if let inset = self.tableviewInset {
 ...
 extension LTSimpleManager: UITableViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if contentTableView.contentOffset.y > 0 - contentTableView.contentInset.top || offsetY > kHeaderHeight - hoverY {
            let contentInset = tableView.contentInset
            tableView.contentOffset = CGPoint(x: 0 - contentInset.left, y: kHeaderHeight - hoverY)
        }
        ...
        if scrollView.contentOffset.y < kHeaderHeight - hoverY {
            for viewController in viewControllers {
                ...
                let contentInset = viewController.glt_scrollView?.contentInset ?? UIEdgeInsets()
                viewController.glt_scrollView?.contentOffset = CGPoint.init(x: 0 - contentInset.left, y: 0 - contentInset.top)
            }
        }
    }
